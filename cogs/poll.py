...
class Poll(commands.Cog):
    ...
    async def poll(self, ctx, time: int, vote: int, title, *options):
        if len(options) > 10:
            await ctx.send(':no_entry: У вас может быть не более **10 вариантов**!')

        elif time <= 15:
            await ctx.send(':no_entry: Укажите время окончания опроса больше, чем **15 минут**!')

        elif 1000000 < vote:
            await ctx.send(':no_entry: Пожалуйста, укажите в опросе максимальное количество голосов, не превышающее **миллион**!')
            ...
            with open('.\\databases\\scheduler.json', 'r') as scheduler_file:
                ...
                if str(ctx.message.channel.id) not in scheduler_data:
                    ...
                    embed = discord.Embed(title=title,
                                          description=f':stopwatch: Опрос закончится через **{time} минут**!',
                                          colour=0xFF0000)
                    ...
                    await ctx.send(f':no_entry: **Канал сейчас занят опросом!**')
                ...
                if str(payload.message_id) in poll_data:
                    ...
                    update_embed = discord.Embed(title=message.embeds[0].title,
                                                 description=f':stopwatch: '
                                                             f'Опрос закончится через **{time_counter} минут**!',
                                                 colour=0xFF0000)
        ...
        with open('.\\databases\\scheduler.json', 'r') as schedule_file:
            ...
            for item in scheduler_data.items():
                ...
                if int(calc_minutes) > item[1]['scheduler_time'] or poll_outcome[1] >= item[1]['max_vote']:
                    if str(message.id) in poll_data:
                        await channel.send(
                            f':tada: **{poll_outcome[0]}** победил **{message.embeds[0].title}** опрос '
                            f'с **{poll_outcome[1]}** голосов!')
